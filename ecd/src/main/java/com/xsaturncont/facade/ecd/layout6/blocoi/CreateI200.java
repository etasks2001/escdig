	...
	public List<_I200> get() throws SQLException {
		...
		for (int mes = 1; mes <= 12; mes++) {
			PreparedStatement ps = connection.prepareStatement(
					"select d.conta, d.dia, d.lote, d.documento, d.valor, d.natureza, d.historico from diario d where d.codigoempresa = "
							+ this.codigoEmpresa + " and d.ano = 19 and d.mes = " + mes
							+ " order by d.dia, d.lote, d.documento, d.valor");
			...
			for (Integer dia : arrayDias) {
				Map<Integer, List<I250Aux>> partidasPorDocumento = new CreateI250AuxDia()
						.getPartidasPorDocumento(mapI250.get(dia));
				...
				for (Integer documento : arrayDocumentos) {
					...
					_I200.setDt_lcto(Util.getCreateDate(2019, mes - 1, dia));
