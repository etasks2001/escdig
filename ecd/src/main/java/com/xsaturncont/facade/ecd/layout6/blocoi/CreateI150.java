...
public class CreateI150 {
	...
	public List<_I150> get() throws ParseException, SQLException {
		...
		Map<String, Double[]> valorMovimentoMensal = new CreateI150ValorMovimentoMensal()
				.getValorMovimentoMensal(this.connection, this.codigoEmpresa);
		Map<String, Double> saldoInicial = new CreateI150SaldoInicial().getSaldoInicial(this.connection,
				this.codigoEmpresa);
		Map<String, Double[]> saldosIniciais = new CreateI150CalcularSaldoMensal().calcularSaldoMensal(saldoInicial,
				valorMovimentoMensal);

		for (int mes = 0; mes < 12; mes++) {
			Calendar dataInicial = Calendar.getInstance();
			dataInicial.set(2019, mes, 1);
