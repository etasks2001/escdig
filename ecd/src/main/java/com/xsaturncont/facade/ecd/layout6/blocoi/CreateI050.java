...
public class CreateI050 {
	...
	PreparedStatement ps = connection.prepareStatement("select conta, tipoconta,descricao from planoconta where codigoempresa = " + this.codigoEmpresa + " and ano = 20 order by conta");

	ResultSet rs = ps.executeQuery();

	PreparedStatement psEcd = connEcd.prepareStatement("select * from I050 where codigoempresa = " + this.codigoEmpresa + " and ano = 20 order by cod_cta");
	...
	while (rsEcd.next()) {
	    ...
	    mapI050Ecd.put(cod_cta, new I050Ecd(sk, ano, codigoempresa, cod_cta, cod_cta_sup, cod_nat, nivel, cod_cta_ref, cod_agl));
