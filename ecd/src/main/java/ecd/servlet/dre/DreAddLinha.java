...
@WebServlet("/dreAddLinha")
public class DreAddLinha extends HttpServlet {
...
//	String ind_grp_bal = request.getParameter("ind_grp_bal");
	...
	String ind_grp_dre = request.getParameter("ind_grp_dre");
...
//	System.out.println("ind_grp_bal     :" + ind_grp_bal);
	...
	System.out.println("ind_grp_dre  :" + ind_grp_dre);
	...
	try {
		...
		ps = connection.prepareStatement(
			"insert into J150 (ano,codigoempresa,nu_ordem,cod_agl,ind_cod_agl,nivel_agl,cod_agl_sup,descr_cod_agl,vl_cta_ini,ind_dc_cta_ini,vl_cta_fin,ind_dc_cta_fin,ind_grp_dre) values (?,?,?,?,?,?,?,?,?,?,?,?,?)");
		ps.setInt(1, ano);
		...
		ps.setString(8, descr_cod_agl);
		ps.setDouble(9, vl_cta_ini);
		ps.setString(10, ind_dc_cta_ini);
		ps.setDouble(11, vl_cta_fin);
		ps.setString(12, ind_dc_cta_fin);
		ps.setString(13, ind_grp_dre);
	    } else {
		ps = connection.prepareStatement(
			"update J150 set nu_ordem=?,cod_agl=?,ind_cod_agl=?,nivel_agl=?,cod_agl_sup=?,descr_cod_agl=?,vl_cta_ini=?,ind_dc_cta_ini=?,vl_cta_fin=?,ind_dc_cta_fin=?,ind_grp_dre=? where sk=?");
		ps.setInt(1, nu_ordem);
		ps.setString(2, cod_agl);
		ps.setString(3, ind_cod_agl);
		ps.setInt(4, nivel_agl);
		ps.setString(5, cod_agl_sup);
		ps.setString(6, descr_cod_agl);
		ps.setDouble(7, vl_cta_ini);
		ps.setString(8, ind_dc_cta_ini);
		ps.setDouble(9, vl_cta_fin);
		ps.setString(10, ind_dc_cta_fin);
		ps.setString(11, ind_grp_dre);
		ps.setString(12, sk);
	...
	PreparedStatement st = c.prepareStatement("select top 1 nu_ordem from J150  where codigoempresa=? and ano=? order by nu_ordem desc");
